...
const EventListSection: React.FC<{title: string, icon: React.ElementType, events: CalendarEvent[], selectedDate: Date, eventRefs: React.MutableRefObject<Map<string, HTMLDivElement | null>>}> = ({title, icon: Icon, events, selectedDate, eventRefs}) => (
    <div className="mb-8">
                        ...
                        <div className="w-1/3 bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border border-slate-200/80 dark:border-slate-700/80 flex flex-col">
                            ...
                            <div className="flex-grow overflow-y-auto pr-2 custom-scrollbar">
                                {(institutionalEvents.length === 0 && civicEvents.length === 0) ? (
                                    ...
                                    <>
                                        {institutionalEvents.length > 0 && <EventListSection title="Eventos Institucionales" icon={Building} events={institutionalEvents} selectedDate={selectedDate} eventRefs={eventRefs} />}
                                        {civicEvents.length > 0 && <EventListSection title="Calendario Cívico y Feriados" icon={Flag} events={civicEvents} selectedDate={selectedDate} eventRefs={eventRefs} />}
                                    </>
                                )}
                            </div>
